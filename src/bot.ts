  ...
  if (link.length > 50 && !arrayMessages.includes(msg)) {
    ...
    await ylist(linkPlaylist, 'url').then((response: Playlist) => {
      ...
      response.data.playlist.map(
        (musica: string) => (filaMusicas = [...filaMusicas, musica]),
      );
    });
    ...
    if (msg.member?.voice.channel) {
      ...
      connection.join().then(connection => {
        const dispatcher = connection.play(
          ytdl(filaMusicas[0], { filter: 'audioonly' }),
        );
...
bot.on('message', msg => {
  try {
    ...
    if (msg.content === '!sortear') {
      const members = msg.member?.voice.channel?.members.map(
        guildMember =>
